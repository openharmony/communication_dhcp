...
DEFINE_DHCPLOG_DHCP_LABEL("DHCPHashTable");
...
int Remove(HashTable *table, uintptr_t key)
{
    ...
    for (HashNode *previous = nullptr; node; previous = node, node = node->next) {
        if (HashEquals(table, key, node->key)) {
            if (previous) {
...
int HashAllocate(HashTable *table, size_t capacity)
{
    ...
    if ((table->nodes = (HashNode **)calloc(capacity, sizeof(HashNode *))) == 0) {
        return HASH_ERROR;
    }
