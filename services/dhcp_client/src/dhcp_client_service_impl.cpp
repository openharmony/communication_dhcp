...
ErrCode DhcpClientServiceImpl::StartDhcpClient(const std::string& ifname, const RouterCfg &config)
{
    DHCP_LOGI("StartDhcpClient ifName:%{public}s bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
    if (!DhcpPermissionUtils::VerifyIsNativeProcess()) {
    ...
    RouterCfg cfg;
    cfg.bssid = config.bssid;
    cfg.prohibitUseCacheIp = config.prohibitUseCacheIp;
    cfg.bIpv6 = config.bIpv6;
    cfg.bSpecifigNetwork = config.bSpecifigNetwork;
    {
        std::lock_guard<std::mutex> autoLock(m_clientServiceMutex);
        auto iter = m_mapClientService.find(ifname);
        if (iter != m_mapClientService.end()) {
            return StartOldClient(ifname, cfg, iter->second);
        }
    }
    return StartNewClient(ifname, cfg);
}

ErrCode DhcpClientServiceImpl::StartOldClient(const std::string& ifname, const RouterConfig &config, DhcpClient &dhcpClient)
{
    DHCP_LOGI("StartOldClient ifname:%{public}s bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
    ...
    if (config.bIpv6) {
        if (dhcpClient.pipv6Client == nullptr) {
            ...
            DHCP_LOGI("StartOldClient new DhcpIpv6Client, ifname:%{public}s, bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
        }
#ifndef OHOS_ARCH_LITE
...
ErrCode DhcpClientServiceImpl::StartNewClient(const std::string& ifname, const RouterConfig &config)
{
    DHCP_LOGI("StartNewClient ifname:%{public}s, bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
    DhcpClient client;
    if (config.bIpv6) {
        DhcpIpv6Client *pipv6Client  = new (std::nothrow)DhcpIpv6Client(ifname);
        if (pipv6Client == nullptr) {
        ...
        DHCP_LOGI("StartNewClient new DhcpIpv6Client, ifname:%{public}s, bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
#ifndef OHOS_ARCH_LITE
        NetManagerStandard::NetsysController::GetInstance().SetIpv6PrivacyExtensions(ifname, DHCP_IPV6_ENABLE);
        NetManagerStandard::NetsysController::GetInstance().SetEnableIpv6(ifname, DHCP_IPV6_ENABLE);
    ...
    DHCP_LOGI("StartNewClient new DhcpClientStateMachine, ifname:%{public}s, bIpv6:%{public}d", ifname.c_str(), config.bIpv6);
