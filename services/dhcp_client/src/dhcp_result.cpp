...
bool PublishDhcpIpv4ResultEvent(const int code, const char *data, const char *ifname)
{
    ...
    if (code == PUBLISH_CODE_TIMEOUT) { // timeout
        ...
        if (DhcpEventResultHandle(code, strData) != OHOS::Wifi::DHCP_OPT_SUCCESS) {
...
int DhcpEventResultHandle(const int code, const std::string &data)
{
    ...
    if (strData.find(OHOS::Wifi::EVENT_DATA_IPV4) != std::string::npos) {
        strFlag = strData.substr(0, (int)OHOS::Wifi::EVENT_DATA_IPV4.size());
        ...
        /* Skip separator ":" */
        strResult = strData.substr((int)OHOS::Wifi::EVENT_DATA_IPV4.size() + 1);
    } else if (strData.find(OHOS::Wifi::EVENT_DATA_IPV6) != std::string::npos) {
        strFlag = strData.substr(0, (int)OHOS::Wifi::EVENT_DATA_IPV6.size());
        if (strFlag != OHOS::Wifi::EVENT_DATA_IPV6) {
            DHCP_LOGE("DhcpEventResultHandle() %{public}s ipv6flag:%{public}s error!", data.c_str(), strFlag.c_str());
            return OHOS::Wifi::DHCP_OPT_FAILED;
        }
        strResult = strData.substr((int)OHOS::Wifi::EVENT_DATA_IPV6.size() + 1);
    } else {
    ...
    if (strFlag == OHOS::Wifi::EVENT_DATA_IPV4) {
        ...
        if (GetDhcpEventIpv4Result(code, vecSplits) != OHOS::Wifi::DHCP_OPT_SUCCESS) {
...
int GetDhcpEventIpv4Result(const int code, const std::vector<std::string> &splits)
{
    ...
    if (splits[OHOS::Wifi::DHCP_NUM_ZERO].empty() || splits[OHOS::Wifi::DHCP_NUM_ONE].empty()) {
        DHCP_LOGE("GetDhcpEventIpv4Result() ifname or time is empty!");
        return OHOS::Wifi::DHCP_OPT_FAILED;
    }

    /* Check field cliIp. */
    if (((code == PUBLISH_CODE_SUCCESS) && (splits[OHOS::Wifi::DHCP_NUM_TWO] == OHOS::Wifi::INVALID_STRING))
    || ((code == PUBLISH_CODE_FAILED) && (splits[OHOS::Wifi::DHCP_NUM_TWO] !=
        OHOS::Wifi::INVALID_STRING))) {
        DHCP_LOGE("GetDhcpEventIpv4Result() code:%{public}d,%{public}s error!", code,
        splits[OHOS::Wifi::DHCP_NUM_TWO].c_str());
        return OHOS::Wifi::DHCP_OPT_FAILED;
    }
    ...
    /* Get success. */
    if (clientImpl->DhcpIpv4ResultSuccess(splits) != OHOS::Wifi::DHCP_OPT_SUCCESS) {
