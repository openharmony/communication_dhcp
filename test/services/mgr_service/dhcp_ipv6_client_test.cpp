...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("ipv6AddrScope2TypeTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("createKernelSocketTest enter!");
    int32_t result = ipv6Client->createKernelSocket();
    LOGE("createKernelSocketTest result(%{public}d)", result);
    EXPECT_EQ(IPV6_RETURN_ZERO, result);
}

HWTEST_F(DhcpIpv6ClientTest, GetIpv6PrefixTest, TestSize.Level1)
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("GetIpv6PrefixTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("GetIpFromS6AddressTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("onIpv6AddressAddEventTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("onIpv6DnsAddEventTest enter!");
    char data[] = "192.168.1.12";
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("onIpv6RouteAddEventTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("getIpv6RouteAddrTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("setSocketFilterTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("parseNdUserOptMessageTest enter!");
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("parseNDRouteMessageTest enter!");
    struct nlmsghdr msg;
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("parseNewneighMessageTest enter!");
    struct nlmsghdr msg;
...
{
    ASSERT_TRUE(ipv6Client != nullptr);
    LOGE("handleKernelEventTest enter!");
